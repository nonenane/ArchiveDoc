...
namespace ArchiveDocPost
{
    public partial class frmList : Form
    {
        ...
        {
            if (DialogResult.OK == new frmAdd() { Text = "Добавить должность" }.ShowDialog())
                get_data();
        }
        ...
        private void btEdit_Click(object sender, EventArgs e)
        {
            if (dgvData.CurrentRow != null && dgvData.CurrentRow.Index != -1 && dtData != null && dtData.DefaultView.Count != 0)
            {
                ...
                if (DialogResult.OK == new frmAdd() { Text = "Редактировать должность", row = row }.ShowDialog())
                    get_data();
            }
        }
        ...
        private void btDelete_Click(object sender, EventArgs e)
        {
            if (dgvData.CurrentRow != null && dgvData.CurrentRow.Index != -1 && dtData != null && dtData.DefaultView.Count != 0)
            {
                ...
                Task<DataTable> task = Config.hCntMain.setPost(id, cName, isActive, true, 0);
                task.Wait();
                ...
                if (result == -2 && isActive)
                {
                    ...
                    {
                        setLog(id, 3);
                        task = Config.hCntMain.setPost(id, cName, !isActive, false, 0);
                        task.Wait();
                        if (task.Result == null)
                        {
                            MessageBox.Show(Config.centralText("При сохранение данных возникли ошибки записи.\nОбратитесь в ОЭЭС\n"), "Сохранение данных", MessageBoxButtons.OK, MessageBoxIcon.Error);
                ...
                if (result == 0 && isActive)
                {
                    ...
                    {
                        setLog(id, 2);
                        task = Config.hCntMain.setPost(id, cName, isActive, true, 1);
                        task.Wait();
                        if (task.Result == null)
                        {
                            MessageBox.Show(Config.centralText("При сохранение данных возникли ошибки записи.\nОбратитесь в ОЭЭС\n"), "Сохранение данных", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void get_data()
        {
            Task.Run(() =>
            {
                ...
                Task<DataTable> task = Config.hCntMain.getPost();
                task.Wait();
                dtData = task.Result;
