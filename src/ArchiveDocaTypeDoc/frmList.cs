    ...
    public partial class frmList : Form
    {
        ...
        private void btDelete_Click(object sender, EventArgs e)
        {
            if (dgvData.CurrentRow != null && dgvData.CurrentRow.Index != -1 && dtData != null && dtData.DefaultView.Count != 0)
            {
                ...
                int npp = (int)dtData.DefaultView[dgvData.CurrentRow.Index]["npp"];
                bool ViewAdd = (bool)dtData.DefaultView[dgvData.CurrentRow.Index]["ViewAdd"];
                bool ViewArchive = (bool)dtData.DefaultView[dgvData.CurrentRow.Index]["ViewArchive"];

                Task<DataTable> task = Config.hCntMain.setTypeDoc(id, cName, npp, ViewAdd, ViewArchive, isActive, true, 0);
                task.Wait();
                ...
                {                    
                    ...
                    if (dlgResult == DialogResult.Yes)
                        ...
                        {
                            setLog(id, 3);
                            task = Config.hCntMain.setTypeDoc(id, cName, npp, ViewAdd, ViewArchive, !isActive, false, 0);
                            task.Wait();
                            if (task.Result == null)
                            {
                                MessageBox.Show(Config.centralText("При сохранение данных возникли ошибки записи.\nОбратитесь в ОЭЭС\n"), "Сохранение данных", MessageBoxButtons.OK, MessageBoxIcon.Error);
                ...
                if (result == 0 && isActive)
                {
                    ...
                    {
                        setLog(id, 2);
                        task = Config.hCntMain.setTypeDoc(id, cName, npp, ViewAdd, ViewArchive, isActive, true, 1);
                        task.Wait();
                        if (task.Result == null)
                        {
                            MessageBox.Show(Config.centralText("При сохранение данных возникли ошибки записи.\nОбратитесь в ОЭЭС\n"), "Сохранение данных", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        {
            tbName.Location = new Point(dgvData.Location.X+cNum.Width+1, tbName.Location.Y);
